...
PICKUPS, DROPOFFS = M.stations(W, n_pick=40)                # 물류 지점(rack 40·dock) — 로봇 수만큼(겹침 회피 여유)
SPAWN = C.package_spawner(PICKUPS, DROPOFFS, every=1, per=1, avoid_busy=True)  # 1.0/t 공급(유휴 최소화) — 백로그는 dock 캡으로 자연 유계
_BR = __import__("random").Random(7)
_bp = _BR.sample(PICKUPS, min(N, len(PICKUPS)))             # 초기 일괄투입도 지점 겹침 없이(샘플링)
