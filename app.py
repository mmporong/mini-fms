...
let RPOS={};                              // 로봇별 렌더 위치(셀 목표로 부드럽게 수렴)
function animate(){                        // 60fps 보간 루프 — 셀→셀 순간이동 대신 미끄러지듯 이동
 const R=STATE.robots||[], cur={}, ids=new Set();
 for(const r of R){ ids.add(r.id); let p=RPOS[r.id];
  if(!p){ p={x:r.x,y:r.y}; RPOS[r.id]=p; }
  p.x+=(r.x-p.x)*0.22; p.y+=(r.y-p.y)*0.22; cur[r.id]=p; }   // 지수 평활(작을수록 부드럽게 따라감)
 for(const id in RPOS) if(!ids.has(id)) delete RPOS[id];
 draw(R, cur);
...
function draw(R, cur){
 ...
 const effs=R.map(r=>r.eff).filter(v=>v!=null);              // 우선순위 색 정규화(낮은 eff=선점=빨강)
 const emn=effs.length?Math.min(...effs):0, erng=(effs.length?Math.max(...effs):1)-emn||1;
 g.clearRect(0,0,W,H);
 ...
 for(const r of R){                                                                                    // 로봇 dot(색=우선순위) + 라벨
  ...
  else { const t=((r.eff??emn)-emn)/erng;                                                              // 0=선점(빨강) .. 1=최저(흰)
...
loadMap().then(()=>{ setInterval(poll,250); poll(); animate(); });   // poll=데이터, animate=부드러운 렌더
</script>
"""
