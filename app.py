...
<div class="wrap">
 ...
 <div class="panel">
  ...
  <div class="legend">
   <span><i class="sw" style="background:#3fb950"></i>이동</span><span><i class="sw" style="background:#e3b341"></i>대기</span>
   <span><i class="sw" style="background:#58a6ff"></i>도착</span><span><i class="sw" style="background:#7d8590"></i>유휴</span>
   <span><i class="sw" style="background:#f85149"></i>고장</span>
   <span><i class="sw" style="background:#1f6feb"></i>픽업 rack</span><span><i class="sw" style="background:#8a6d1f"></i>배송 dock</span>
   <span><i class="sw" style="background:#f0d060"></i>적재 물류</span>
   <span style="color:#8b949e">로봇: <b>번호</b>·<span style="color:#f0d060">p우선순위</span></span>
  </div>
  <div class="logs">
   <div>
...
function draw(R){
 ...
 for(const r of R){                                                                                    // 로봇 dot + 라벨(모드별)
  const X=px(r.x), Y=py(r.y), rad=Math.max(2,cs*0.46);
  g.fillStyle=COLOR[r.status]||'#e6edf3'; g.beginPath(); g.arc(X,Y,rad,0,7); g.fill();
...
loadMap().then(()=>{ setInterval(poll,250); poll(); });
</script>
"""
